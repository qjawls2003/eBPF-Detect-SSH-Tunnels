...
static int probe_entry_connect(void *ctx, struct sockaddr_in6 *addr) {
  __u64 id = bpf_get_current_pid_tgid();
  pid_t pid = id >> 32;
  //pid_t tid = (__u32)id;

  bpf_map_update_elem(&tunnels, &pid, &addr, BPF_ANY);
  return 0;
};

static int probe_return_connect(void *ctx, int ret) {
  __u64 id = bpf_get_current_pid_tgid();
  pid_t pid = id >> 32;
  //pid_t tid = (__u32)id;
  uid_t uid = bpf_get_current_uid_gid() & 0xFFFFFFFF;
  ...
  addrpp = bpf_map_lookup_elem(&tunnels, &pid);
  if (!addrpp)
    return 0;
...
int tp_sys_enter_connect(struct trace_event_raw_sys_enter *ctx) {
  return probe_entry_connect(ctx, (struct sockaddr_in6*)ctx->args[1]);
}

SEC("tp/syscalls/sys_exit_connect")
int tp_sys_exit_connect(struct trace_event_raw_sys_exit *ctx) {
  return probe_return_connect(ctx, (int)ctx->ret);
}
